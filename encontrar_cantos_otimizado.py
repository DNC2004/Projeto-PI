...
def encontrar_cantos(img_path):
    ...
    #Preparar Imagem / Melhora a robustez
    ...
    # Expandir as regiões para detetar melhor o tabuleiro
    mask_r_d = cv2.dilate(mask_red, kernel, iterations = 1)
    mask_w_d = cv2.dilate(mask_white, kernel, iterations = 1)

    inter = cv2.bitwise_and(mask_r_d, mask_w_d) # Área com overlap

    # Regiões vermelhas e brancas
    num_r, labels_r = cv2.connectedComponents(mask_r_d, connectivity = 8)
    num_w, labels_w = cv2.connectedComponents(mask_w_d)
    
    #  Ficamos apenas com os componentes que que se intercetam
    # Evita contabilizar regiões com as mesmas cores que o tabuleiro mas que não sejam o tabuleiro
    ...
    """ 
    2. ENCONTRAR A ÁREA DE JOGO
    """
    """"""

    # Máscara nos componentes que se toquem 
    ...
    # Masc. com a área do tabuleiro
    mask_tiles_touch = cv2.bitwise_or(mask_r_touch, mask_w_touch)
    
    mask = cv2.erode(mask_tiles_touch, kernel, iterations=1)
    ...
    # Maior contorno == Tabuleiro
    largest = max(contornos, key=cv2.contourArea)
    ...
    # Escolhe as quatro maiores arestas -- Área possível do tabuleiro 
    segments.sort(key=lambda x: x[0], reverse=True)
    top4 = segments[:4]

    # Determina o ponto de interseção entre duas retas
    ...
    # Interseções entre os maiores pares de retas
    intersections = []
    ...
    # Escolher as interseções mais perto deste
    ...
    for (_, p1, p2) in top4:
        cv2.line(overlay, tuple(p1), tuple(p2), (0,0,255), 10)

    alpha = 0.6
    output = cv2.addWeighted(img_rgb, 1 - alpha, overlay, alpha, 0)

    """ 3. TRANSFORMAR PARA A PERSPETIVA """
    """"""
    ...
    # Transformar para a perspectiva final
    ...
    return warped_export

if __name__ == "__main__":
    encontrar_cantos(caminho_img)